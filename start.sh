...
echo -e "${YELLOW}[1/3] Starting ML Microservice (FastAPI + IsolationForest)...${NC}"
...
# 2. Node.js Gateway
echo -e "${MAGENTA}[2/3] Starting Node.js Gateway (Express)...${NC}"
(cd "$ROOT/backend" && npx tsx src/index.ts) &
NODE_PID=$!
sleep 2

# 3. React Vite Frontend
echo -e "${GREEN}[3/3] Starting React Dashboard (Vite)...${NC}"
...
echo -e "${GRAY}  PIDs: ML=$ML_PID  Node=$NODE_PID  React=$REACT_PID${NC}"
echo -e "${GRAY}  Press Ctrl+C to stop all.${NC}"
echo ""

# Keep alive — stop all on Ctrl+C
trap "kill $ML_PID $NODE_PID $REACT_PID 2>/dev/null; echo 'All services stopped.'; exit 0" INT
wait
